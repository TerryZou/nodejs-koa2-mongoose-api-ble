...
const BtConnect = require("../bll/BtConnect_bll");

//查询连接测试记录
exports.query = async(ctx, next) => {
	var codes = apiCode.bt_connect_query.codes;
	...
	try {
		...
		if(isgo) {
			var data = await BtConnect.getRecordsBySearch({
				mac: mac,
				name: name,
				mi: mi,
				flag: flag,
...
exports.query_list = async(ctx, next) => {
	...
	try {
		...
		if(isgo) {
			var data = await BtConnect.getRecordsListBySearchs(params);
			switch(data.status) {
				case 1:
					result.data = data.data;
					result.status = codes.success;
...
exports.query_export = async(ctx, next) => {
	var codes = apiCode.bt_connect_query_export.codes;
	var result = new Object();
	try {
		var mac = ctx.request.body.mac;
		var name = ctx.request.body.name;
		...
		if(isgo) {
			var data = await BtConnect.exportRecords({
				mac: mac,
				name: name,
				mi: mi,
				flag: flag,
...
exports.query_list_export = async(ctx, next) => {
	...
	try {
		...
		if(isgo) {
			var data = await BtConnect.exportRecordsList(params);
			switch(data.status) {
				case 1:
					result.data = data.data;
					result.status = codes.success;
...
exports.result = async(ctx, next) => {
	var codes = apiCode.bt_connect_result.codes;
	var result = new Object();
	try {
		var mac = ctx.request.body.mac;
		var name = ctx.request.body.name;
		...
		if(isgo) {
			var data = await BtConnect.getResultBySearch({
				mac: mac,
				name: name,
				mi: mi,
				flag: flag,
...
exports.result_export = async(ctx, next) => {
	var codes = apiCode.bt_connect_result_export.codes;
	var result = new Object();
	try {
		var mac = ctx.request.body.mac;
		var name = ctx.request.body.name;
		...
		if(isgo) {
			var data = await BtConnect.exportResult({
				mac: mac,
				name: name,
				mi: mi,
				flag: flag,
...
exports.result_list = async(ctx, next) => {
	...
	try {
		...
		if(isgo) {
			var data = await BtConnect.getResultBySearch(params);
			switch(data.status) {
				case 1:
					result.data = data.data;
					result.status = codes.success;
...
exports.result_list_export = async(ctx, next) => {
	...
	try {
		...
		if(isgo) {
			var data = await BtConnect.exportResults(params);
			switch(data.status) {
				case 1:
					result.data = data.data;
					result.status = codes.success;
